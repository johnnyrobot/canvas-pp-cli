...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("CANVAS_API_TOKEN"); v != "" {
		...
		cfg.markEnvOverride("CanvasApiToken")
		cfg.AuthSource = "env:CANVAS_API_TOKEN"
		cfg.CredentialSource = "env:CANVAS_API_TOKEN"
	}
	if v := os.Getenv("CANVAS_ACCESS_TOKEN"); v != "" {
		cfg.AccessToken = v
		cfg.markEnvOverride("AccessToken")
		cfg.AuthSource = "env:CANVAS_ACCESS_TOKEN"
		cfg.CredentialSource = "env:CANVAS_ACCESS_TOKEN"
	}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CANVAS_CONFIG"); path != "" {
...
func (c *Config) AuthHeader() string {
	...
	if c.CanvasApiToken != "" {
		...
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_token":        c.CanvasApiToken,
			"CANVAS_API_TOKEN": c.CanvasApiToken,
			"token":            c.CanvasApiToken,
		})
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CANVAS_ACCESS_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"access_token":        c.AccessToken,
			"CANVAS_ACCESS_TOKEN": c.AccessToken,
			"token":               c.AccessToken,
		})
	}
	return ""
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		CanvasApiToken: c.CanvasApiToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.CanvasApiToken = creds.CanvasApiToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.CanvasApiToken = ""
	delete(c.envOverrides, "CanvasApiToken")
	c.updateFileConfigField("CanvasApiToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["CanvasApiToken"] {
			out.CanvasApiToken = c.fileConfig.CanvasApiToken
		}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "CanvasApiToken":
		c.fileConfig.CanvasApiToken = c.CanvasApiToken
	}
}
