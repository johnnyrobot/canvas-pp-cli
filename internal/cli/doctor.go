		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set credentials with: export CANVAS_API_TOKEN=\"your-token-here\" CANVAS_ACCESS_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("CANVAS_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CANVAS_ACCESS_TOKEN")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CANVAS_ACCESS_TOKEN")
			}
			switch {
