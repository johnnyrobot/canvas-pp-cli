...
func newOutcomeResultsOutcomeOrderCmd(flags *rootFlags) *cobra.Command {
	var stdinBody bool
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 {
				...
				if partialFailure != nil {
					fmt.Fprintf(os.Stderr, "warning: partial failure detected in %s response: %s\n", "outcome_results", partialFailure.Message)
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "outcome_results", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "outcome_results", partialFailure.Message))
						}
						return nil
					}
				} else {
