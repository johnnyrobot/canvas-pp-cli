...
func newLtiRegistrationsGetByUtidCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 2 || args[1] == "" {
				return usageErr(fmt.Errorf("utid is required\nUsage: %s <%s>", cmd.CommandPath(), "utid"))
			}
			path = replacePathParam(path, "utid", args[1])
