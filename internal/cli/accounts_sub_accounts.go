...
func newAccountsSubAccountsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().BoolVar(&flagRecursive, "recursive", false, "If true, the entire account tree underneath this account will be returned (though still paginated).")
	cmd.Flags().StringVar(&flagOrder, "order", "", "Sorts the accounts by id or name. Only applies when recursive is false. Defaults to id. Allowed values: id, name")
