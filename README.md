...
`canvas-pp-cli` is all you need — it's the only binary this project ships. There is deliberately no MCP server; see [Chat apps that can't run commands](#chat-apps-that-cant-run-commands).
...
>
> Use the CLI directly from the shell from now on — every command takes `--agent` for JSON output. There is no MCP server to set up.

That's the whole setup. There is no daemon and no config file to register — the CLI is the integration.

## Use with coding agents

**If your agent can run shell commands, just let it run the CLI.** That covers Claude Code, Codex, Google Antigravity, Cursor, Aider, and anything else with terminal access. No wiring, nothing to keep running.
...
## Chat apps that can't run commands

Not supported, by design. `canvas-pp-cli` is the only binary this project ships, and it needs a shell.

Earlier releases included a companion MCP server (`canvas-pp-mcp`) for Claude Desktop and ChatGPT Desktop. It has been removed. If you are on a host with no terminal, use a shell-capable agent instead — see [Use with coding agents](#use-with-coding-agents).

## Authentication

Canvas uses a per-institution base URL and a Bearer access token. Set CANVAS_BASE_URL to your instance host (e.g. https://your-school.instructure.com) and CANVAS_API_TOKEN to a token from Account > Settings > New Access Token. Tokens are user-scoped, so the CLI can only do what your Canvas account can.

## Quick Start

```bash
# Verify config and connectivity before anything else (works without a token).
canvas-pp-cli doctor --dry-run
...
canvas-pp-cli account-domain-lookups

# JSON for scripting and agents
canvas-pp-cli account-domain-lookups --json

# Filter to specific fields
canvas-pp-cli account-domain-lookups --json --select id,name,status

# Dry run — show the request without sending
canvas-pp-cli account-domain-lookups --dry-run

# Agent mode — JSON + compact + no prompts in one flag
canvas-pp-cli account-domain-lookups --agent
```
...
Run `canvas-pp-cli doctor` to see the resolved config, data, state, and cache directories. The platform-default config path is `~/.config/canvas-pp-cli/config.toml`; `--home`, `CANVAS_HOME`, and per-kind env vars can relocate it.
...
- **roster / at-risk return empty** — Run 'canvas-pp-cli sync --resources courses,enrollments,users,submissions' first to populate the local mirror.
...
- [**vishalsachdev/canvas-pp-mcp**](https://github.com/vishalsachdev/canvas-pp-mcp) — Python (80 stars)
- [**KTH/canvas-api**](https://github.com/KTH/canvas-api) — TypeScript (40 stars)
- [**mbund/canvas-pp-cli**](https://github.com/mbund/canvas-pp-cli) — Rust (30 stars)
